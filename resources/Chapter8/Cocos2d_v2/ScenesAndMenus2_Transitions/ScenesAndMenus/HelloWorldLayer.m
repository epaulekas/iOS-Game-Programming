...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCLabelTTF *label1 = [CCLabelTTF labelWithString:@"Button 1" fontName:@"Marker Felt" fontSize:64];
        CCLabelTTF *label2 = [CCLabelTTF labelWithString:@"Button 2" fontName:@"Arial" fontSize:30];
        
        CCMenuItem *button1 = [CCMenuItemLabel itemWithLabel:label1 target:self selector:@selector(buttonMethod1:)];
         CCMenuItem *button2 = [CCMenuItemLabel itemWithLabel:label2 target:self selector:@selector(buttonMethod1:)];
        
        button1.tag = 12345;
        
        //CCMenuItem *button1 = [CCMenuItemFont itemWithString:@"My Button1" target:self selector:@selector(button1Method)];
...
-(void) buttonMethod1:(id)sender {
    
    CCLOG(@"button 1 pressed %@ ", sender );
    
    CCMenuItem* touchedItem = (CCMenuItem*)sender;
    
    if (touchedItem.tag == 12345) {
        
        CCLOG(@"button 1 was pressed" );
    } else  {
        
        CCLOG(@"another button was pressed" );
    }
...
-(void) replaceWithMyself {
    ...
    if (theCount == 1) {
        ...
        CCTransitionProgressHorizontal *transition = [CCTransitionProgressHorizontal transitionWithDuration:2 scene:[HelloWorldLayer scene]  ];
        [[CCDirector sharedDirector] replaceScene:transition];
    } 
    else if (theCount == 6) {
        CCTransitionProgressRadialCW *transition = [CCTransitionProgressRadialCW transitionWithDuration:2 scene:[HelloWorldLayer scene] ];
        [[CCDirector sharedDirector] replaceScene:transition];
    } 
    else if (theCount == 7) {
